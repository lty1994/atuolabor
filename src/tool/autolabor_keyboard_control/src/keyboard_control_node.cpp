...
void KeyboardControl::parseKeyboard(){
  while (true) {
    ...
    if (ev_.type == EV_KEY){//value值按下键时为1,松开为0
...
bool KeyboardControl::init(){//绑定键盘,获取输入到上下文,fd_
  const char path[] = "/dev/input/by-path";
  DIR *dev_dir = opendir(path);
  struct dirent *entry;//定义输入文件对象
  if (dev_dir == NULL){
    return false;
  }
...
void KeyboardControl::run(){
  if (init()){
    ...
    ros::spin();//进入循环处理,知道ros::ok()返回false结束;
  }
}
}

int main(int argc, char *argv[]){
  ros::init(argc, argv, "keyboard_control_node");//1.指定节点名称
  autolabor_tool::KeyboardControl keyboard_control;//实例化节点
